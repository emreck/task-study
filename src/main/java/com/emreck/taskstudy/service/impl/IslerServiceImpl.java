...
public class IslerServiceImpl implements IsService {
 ...
 /*  constracture ile artısı : final tanımlamasıyla o nesnenin run time da değişmesini engellemiş oluyorsun. atama yapmayı engelliyor.
    ...
    public IslerDto save(IslerDto is) {
        ...
        is.setId(islerdb.getId());
       return is;//tersini yaptık.
    ...
    public IslerDto getById(Long id) {
        Isler isler = isRepository.getOne(id);
        return modelMapper.map(isler,IslerDto.class);
    }
    ...
    public TPage<IslerDto> getAllPageable(Pageable pageable) {//amaç dtoya dönüştürüp geri vermek.
      ...
      IslerDto[] dtos = modelMapper.map(data.getContent(),IslerDto[].class);
        ...
        TPage<IslerDto> response = new TPage<>();
        response.setStat(data,Arrays.asList(modelMapper.map(data.getContent(),IslerDto[].class)));
         return response;
    }

    @Override
    public Boolean delete(Long islerId) {
     isRepository.deleteById(islerId);
     return true;
    }

    @Override
    public IslerDto update(Long id, IslerDto project) {
        return null;
    }
}
